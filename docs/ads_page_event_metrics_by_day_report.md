# Ads Page Event Metrics by Day Report

This table contains TikTok Ads page event metrics at the ad and day level, with one row per ad and stat_time_day. It provides detailed page event and conversion metrics for each ad, enabling granular performance analysis and reporting.
...
| ...         | FLOAT64   | All page event and conversion metrics, cast to FLOAT64 (see SQL for full list) |
| _fivetran_synced | TIMESTAMP | ETL load timestamp                                                    |

## How to Use This Table

- **Performance Analysis**: Aggregate or filter by `ad_id` or `stat_time_day` to analyze ad page event performance over time.
- **KPI Tracking**: Use event and conversion columns to calculate key metrics such as conversion rates, ROAS, and more.
...
- All event and conversion metrics are cast to FLOAT64 for consistency.
